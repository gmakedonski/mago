...
const rootCompany = {
  ...
  firebase_key: '',
  help_page: '/help_support',
  terms_condition_page: '/terms_and_condition',
  vod_subset_nr: 200,
...
const adminGroup = {name: 'Admin', code: 'admin', isavailable: 1, company_id: 1};
const adminUser = {
  username: 'admin',
    ...
    db.users.findOne({
      ...
      return sequelize_t.sequelize.transaction(function (t) {
        return db.settings.create(settings_object, {transaction: t}).then(function (new_company) {
          ...
          let channel_stream_sources = {
            company_id: new_company.id,
            stream_source: 'Live primary source - ' + new_company.company_name
          };
          let vod_stream_sources = {
            company_id: new_company.id,
            description: 'VOD primary source - ' + new_company.company_name
          };
          //prepare object for device_menu
          let device_menus = require(path.resolve("./config/defaultvalues/device_menu.json"));
          device_menus.forEach(function (element) {
            delete element.id
          });
          device_menus.forEach(function (element) {
            element.company_id = new_company.id
          });
          ...
          email_templates.forEach(function (element) {
            delete element.id
          });
          email_templates.forEach(function (element) {
            element.company_id = new_company.id
              ...
              return db.device_menu.bulkCreate(device_menus, {transaction: t}).then(function (device_menu) {
                return db.advanced_settings.create(advanced_settings, {transaction: t}).then(function (advanced_settings) {
                  return db.email_templates.bulkCreate(email_templates, {transaction: t}).then(function (email_templates) {
                    ...
                    return db.groups.create(admin_group, {transaction: t}).then(async (admin_data) => {
                      // insert default players
                      const platforms = await db.app_group.findAll();
                      let deafultPlayersData = platforms.map(platform => ({
                        company_id: new_company_data.id,
                        player_name: 'default',
                        default: 1,
                        app_id: platform.app_id
                      }));
                      await db.media_player.bulkCreate(deafultPlayersData, { transaction: t });
                      // add user related to the company
...
exports.createAdminCompany = function () {
  return new Promise(function (resolve, reject) {
    db.settings.findOne({
      where: {id: 1}
    }).then(function (result) {
      ...
      winston.info('Creating admin company...');
      ...
      return sequelize_t.sequelize.transaction(function (t) {
        return db.settings.create(adminCompany, {transaction: t}).then(function (new_company) {
          new_company_data = new_company; //load the data of the new company
          ...
          const channel_stream_sources = {
            company_id: new_company.id,
            stream_source: 'Live primary source - ' + new_company.company_name
          };
          const vod_stream_sources = {
            company_id: new_company.id,
            description: 'VOD primary source - ' + new_company.company_name
          };
          ...
          return db.channel_stream_source.create(channel_stream_sources, {transaction: t}).then(function (new_company) {
            return db.vod_stream_source.create(vod_stream_sources, {transaction: t}).then(function (vod_source) {
              return db.device_menu.bulkCreate(device_menus, {transaction: t}).then(function (device_menu) {
                return db.advanced_settings.bulkCreate(advanced_settings, {transaction: t}).then(function (advanced_settings) {
                  return db.email_templates.bulkCreate(email_templates, {transaction: t}).then(function (email_templates) {
                    return db.groups.create(adminGroup, {transaction: t}).then(function (adminGroup) {
                      adminUser.group_id = adminGroup.id;
                      let user = db.users.build(adminUser);
                      user.salt = user.makeSalt();
                      return user.save({transaction: t}).then(function (new_user) {
                        ownerUser = new_user;
            ...
            winston.info('Admin company is created successfully')
            settings.updateCompanySettings(new_company_data);
            settings.updateAdvancedSettings(advanced_settings);
            resolve({company: new_company_data, owner: ownerUser});
          }).catch(function (err) {
            winston.error('Admin company creation failed with error: ', err)
            reject({code: 500});
          });
